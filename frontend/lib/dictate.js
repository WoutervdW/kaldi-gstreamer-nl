(function(window){
	...
	var REFERENCE_HANDLER = "http://127.0.0.1:8080/client/dynamic/reference";
	...
	var Dictate = function(cfg) {
		...
		var audioContext;
		var recorder;
		// Initialized by  init() #startListening()
		var ws;
		var intervalKey;
		...
		this.loadFromCache = function() {
			...
			var was = new WebSocket(url);

			was.onmessage = function(e) {
				var data = e.data;
			...
			was.onopen = function(e) {

				was.send("{ \"load_cache\": 1 }");
			}
		}
		...
		function createWebSocket(add_on_open = true) {
