...
class DecoderPipeline2Tests(unittest.TestCase):
    ...
    def test8k(self):
        self.decoder_pipeline.init_request(
            "test8k",
            "audio/x-raw, layout=(string)interleaved, rate=(int)8000, format=(string)S16LE, channels=(int)1",
        )
    ...
    def testWav(self):
        ...
        while not self.finished:
            time.sleep(1)
        self.assertEqual(
            ["see on esimene lause pärast mida tuleb vaikus", "nüüd tuleb teine lause"],
            self.final_hyps,
        )
