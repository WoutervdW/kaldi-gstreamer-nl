...
class DecoderPipelineTests(unittest.TestCase):
    ...
    def testOgg(self):
        ...
        while not self.finished:
            time.sleep(1)
        self.assertEqual(
            "see on esimene lause <#s> see on teine lause <#s>".split(), self.words
        )
    ...
    def __testDecoder(self):
        ...
        self.assertItemsEqual(
            ["see", "on", "teine", "lause", "<#s>"], words, "Recognition result"
        )
        ...
        while not finished[0]:
            time.sleep(1)
        self.assertItemsEqual(
            "see on esimene lause <#s> see on teine lause <#s>".split(),
            words,
            "Recognition result",
        )
