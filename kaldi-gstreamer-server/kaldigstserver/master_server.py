...
class Application(tornado.web.Application):
    ...
    def send_status_update_single(self, ws):
        status = dict(
            num_workers_available=len(self.available_workers),
...
def content_type_to_caps(content_type):
    ...
    if media_type in ["audio/x-raw", "audio/x-raw-int"]:
        ...
        return "%s, %s" % (
            media_type,
            ", ".join(
                ["%s=%s" % (key, value) for (key, value) in attributes.iteritems()]
            ),
        )
    else:
        return content_type
...
class HttpChunkedRecognizeHandler(tornado.web.RequestHandler):
    ...
    def data_received(self, chunk):
        ...
        logging.debug(
            "%s: Forwarding client message of length %d to worker"
            % (self.id, len(chunk))
        )
...
class ReferenceHandler(tornado.web.RequestHandler):
    def post(self, *args, **kwargs):
        ...
        if content_id:
            ...
            self.application.save_reference(
                content_id,
                dict(
                    content=content,
                    user_id=user_id,
            ...
            logging.info(
                "Received reference text for content %s and user %s"
                % (content_id, user_id)
            )
            self.set_header("Access-Control-Allow-Origin", "*")
        else:
...
class DecoderSocketHandler(tornado.websocket.WebSocketHandler):
    ...
    def open(self):
        ...
        logging.info(
            "%s: Request arguments: %s"
            % (
                self.id,
                " ".join(
                    [
                        '%s="%s"' % (a, self.get_argument(a))
        ...
        try:
            ...
            logging.warn("%s: No worker available for client request" % self.id)
            event = dict(
                status=common.STATUS_NOT_AVAILABLE,
    ...
    def on_message(self, message):
        assert self.worker is not None
        logging.info(
            "%s: Forwarding client message (%s) of length %d to worker"
