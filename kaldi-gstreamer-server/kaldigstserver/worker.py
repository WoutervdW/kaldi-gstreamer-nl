...
class ServerWebsocket(WebSocketClient):
    ...
    def received_message(self, m):
        ...
        if self.state == self.__class__.STATE_CONNECTED:
            ...
            logger.info("Loading text message with content: " + str(m))
            if "load_cache" in props:
                self.decoder_pipeline.load_from_cache()
    ...
    def _on_result(self, result, final):
        logger.info("worker received partial results!")
        try:
            self._increment_num_processing(1)
            if final:
                logger.info("skipping because final")
                # final results are handled by _on_full_result()
                return
            self.last_decoder_message = time.time()
            if self.last_partial_result == result:
                logger.info("same as previous result, skipping..")
                return
            ...
            if processed_transcripts:
                logger.info("Received transacripts: " + processed_transcripts[0])
                logger.info("%s: Postprocessing done." % self.request_id)
                event = dict(
                    status=common.STATUS_SUCCESS,
                    segment=self.num_segments,
                    result=dict(
                        hypotheses=[dict(transcript=processed_transcripts[0])],
                        final=final,
                    ),
                )
                try:
            ...
            logger.info("something failed in on_result")
            self._increment_num_processing(-1)
    ...
    def _on_word(self, word):
        try:
            ...
            if word != "<#s>":
                ...
                processed_transcript = yield self.post_process(
                    self.partial_transcript, blocking=True
                )
                logger.info("%s: Postprocessing done." % self.request_id)
                event = dict(
                    status=common.STATUS_SUCCESS,
    ...
    def post_process(self, texts, blocking=False):
        if self.post_processor:
            ...
            #     timeout=0.0
            try:
                with (yield self.post_processor_lock.acquire(None)):
                    result = []
                    for text in texts:
