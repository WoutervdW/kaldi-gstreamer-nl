...
class DecoderPipeline(object):
    def __init__(self, conf={}):
        logger.info("setting foo")
        r.set("foo", "bar")
        logger.info("all done")
        logger.info("foo is now: " + r.get("foo"))
    ...
    def create_pipeline(self, conf):
        ...
        if not self.asr:
            ...
            if gst_plugin_path:
                print >>sys.stderr, "Couldn't find onlinegmmdecodefaster element at %s. " "If it's not the right path, try to set GST_PLUGIN_PATH to the right one, and retry. " "You can also try to run the following command: " "'GST_PLUGIN_PATH=%s gst-inspect-1.0 onlinegmmdecodefaster'." % (
                    gst_plugin_path,
                    gst_plugin_path,
                )
            else:
    ...
    def _read_buffer_list(self):
        buffer_cache = r.get("buffer-cache")
        if buffer_cache is None:
            return Gst.BufferList.new()
    ...
    def process_data(self, data):
        ...
        # buf = Gst.Buffer.new_allocate(None, len(data), None) # TODO read buffer from Redis

        # buf.fill(0, data)
        # self.appsrc.emit("push-buffer", buf)
        buffer_list = self._read_buffer_list()
        buf = Gst.Buffer.new_allocate(None, len(data), None)
        buf.fill(0, data)
        buffer_list.append(buf)
        logger.info("Worker received  a")
        self.appsrc.emit("push-buffer-list", buffer_list)
