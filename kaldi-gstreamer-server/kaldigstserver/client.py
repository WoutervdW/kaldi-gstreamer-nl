    ...
    def received_message(self, m):
        ...
        if response["status"] == 0:
            if "result" in response:
                trans = response["result"]["hypotheses"][0]["transcript"].encode(
                    "utf-8"
                )
            ...
            print >> sys.stderr, "Received error from server (status %d)" % response[
                "status"
            ]
...
def main():
    ...
    if content_type == "" and args.audiofile.name.endswith(".raw"):
        content_type = (
            "audio/x-raw, layout=(string)interleaved, rate=(int)%d, format=(string)S16LE, channels=(int)1"
            % (args.rate / 2)
        )

    ws = MyClient(
        args.audiofile,
        args.uri + "?%s" % (urllib.urlencode([("content-type", content_type)])),
        byterate=args.rate,
