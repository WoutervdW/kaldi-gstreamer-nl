...
1. Unzip models.zip in the root directory

2. run `docker build . -t some-tag`

3. Replace the tag of `kaldi-worker` in `docker-compose` with your local image

4. Run `docker-compose up` this will start a redis on port 6379 and the kaldi server on port 8080

5. Allow your browser to access files by using `file://` within a script. [instructions](https://docs.bentley.com/LiveContent/web/ContextCapture%20Help-v10/en/GUID-0A9BDD35-5882-46A0-BF43-42349D10E881.html)

6. Open `frontend/demos/mob.html` in your browser.



## Docker kaldi gstreamer server documentation:
